package tfe

import (
	"fmt"
	"math/rand"
	"testing"
	"time"

	tfe "github.com/hashicorp/go-tfe"
	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource"
	"github.com/hashicorp/terraform-plugin-sdk/v2/terraform"
)

// Testing this resource requires a valid public GPG key because the TFE
// API will return an HTTP 422 for an invalid GPG key. This is HashiCorp's
// public GPG key as of the time of writing.
var sampleAsciiArmor = `-----BEGIN PGP PUBLIC KEY BLOCK-----


=7pIB
-----END PGP PUBLIC KEY BLOCK-----`

func TestAccTFERegistryGPGKey_basic(t *testing.T) {
	gpgKey := &tfe.GPGKey{}
	rInt := rand.New(rand.NewSource(time.Now().UnixNano())).Int()

	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV5ProviderFactories: testAccMuxedProviders,
		CheckDestroy:             testAccCheckTFESSHKeyDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccTFERegistryGPGKey_basic(rInt),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckTFERegistryGPGKeyExists(
						"tfe_registry_gpg_key.foobar", gpgKey),
					// The newline needs to be concatenated to the end of sampleAsciiArmor here
					// because the heredoc syntax in Terraform, which testAccTFERegistryGPGKey_basic
					// uses to specify the ascii_armor attribute, adds a single newline to the
					// end of the string specified by the user.
					resource.TestCheckResourceAttr(
						"tfe_registry_gpg_key.foobar", "ascii_armor", sampleAsciiArmor+"\n"),
				),
			},
		},
	})
}

func testAccCheckTFERegistryGPGKeyExists(
	n string, gpgKey *tfe.GPGKey) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		tfeClient := testAccProvider.Meta().(*tfe.Client)

		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("Not found: %s", n)
		}

		gpgKeyID := tfe.GPGKeyID{}

		providerNamespace, ok := rs.Primary.Attributes["provider_namespace"]
		if !ok {
			return fmt.Errorf("No provider_namespace is set")
		}

		keyID, ok := rs.Primary.Attributes["key_id"]
		if !ok {
			return fmt.Errorf("No key_id is set")
		}

		// Even though this specific test doesn't require the instance ID and
		// we construct the GPGKeyID for the GPGKeys.Read call from other
		// resource attributes, it doesn't hurt to read the instance ID and
		// validate that one is set.
		if rs.Primary.ID == "" {
			return fmt.Errorf("No instance ID is set")
		}

		gpgKeyID.RegistryName = tfe.RegistryName("private")
		gpgKeyID.Namespace = providerNamespace
		gpgKeyID.KeyID = keyID

		gk, err := tfeClient.GPGKeys.Read(ctx, gpgKeyID)
		if err != nil {
			return err
		}

		if gk == nil {
			return fmt.Errorf("GPG key not found")
		}

		*gpgKey = *gk

		return nil
	}
}

func testAccTFERegistryGPGKey_basic(rInt int) string {
	return fmt.Sprintf(`
resource "tfe_organization" "foobar" {
  name  = "tst-terraform-%d"
  email = "admin@company.com"
}

resource "tfe_registry_gpg_key" "foobar" {
  provider_namespace = tfe_organization.foobar.name
  ascii_armor        = <<EOT
%s
EOT
}`, rInt, sampleAsciiArmor)
}
